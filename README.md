...
- En `PUSSY_XUSDC`, el cambio diario se toma de `change_24hr` (Bloks) y NO aplica la regla extrema del -50%.
...
sudo npm i -g pm2
git clone https://github.com/networkxpr-max/1.git
cd 1
npm install
cp .env.example .env
```
...
- `asignacionesMercado` (máximo total 100%)
	...
	- `PUSSY_XMD`: porcentaje para comprar PUSSY
	  - referencia externa de información: par `PUSSY/XUSDC` (Bloks)
...
- `bloqueoCompraUltimosDias` (0 = compra todo el mes)
	- cuando un mercado se bloquea por calendario, su % se reparte en partes iguales entre los mercados activos.
