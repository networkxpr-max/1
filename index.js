...
const truncateToDecimals = (value, decimals = 2) => {
...
const pussyMarketSymbol = 'PUSSY_XMD';
const pussyTokenApiUrl = 'https://www.api.bloks.io/proton/tokens/PUSSY-proton-xprpussydao';
const alcorSwapRouterUrl = 'https://proton.alcor.exchange/api/v2/swapRouter/getRoute';
const alcorSwapAccount = 'swap.alcor';
const xmdTokenContract = 'xmd.token';
...
const fetchPussyMarketInfo = async () => {
  const data = await fetchJson(pussyTokenApiUrl);
  const token = Array.isArray(data) ? data[0] : null;
  const change = Number(token?.price?.change_24hr);

  const pussyXusdcPair = Array.isArray(token?.pairs)
    ? token.pairs.find(p => String(p?.id || '').toUpperCase() === 'PUSSY/XUSDC')
    : null;
  const xusdcPrice = Number(pussyXusdcPair?.quote?.price);

  return {
    change24h: Number.isFinite(change) ? change : null,
    xusdcPrice: Number.isFinite(xusdcPrice) ? xusdcPrice : null,
  };
};

const formatXmdQuantity = (amount) => {
  const normalized = Number(amount);
  if (!Number.isFinite(normalized) || normalized <= 0) return null;
  return `${normalized.toFixed(6)} XMD`;
};

const fetchPussySwapRoute = async (xmdAmount) => {
  const amount = Number(xmdAmount);
  if (!Number.isFinite(amount) || amount <= 0) {
    throw new Error(`Monto inválido para swap de PUSSY: ${xmdAmount}`);
  }

  const params = new URLSearchParams({
    trade_type: 'EXACT_INPUT',
    input: 'xmd-xmd.token',
    output: 'pussy-xprpussydao',
  ...
  return fetchJson(`${alcorSwapRouterUrl}?${params.toString()}`);
};

const executePussyBuyViaSwap = async (xmdAmount) => {
  const routeData = await fetchPussySwapRoute(xmdAmount);
  const swaps = Array.isArray(routeData?.swaps) ? routeData.swaps : [];

  if (!swaps.length) {
    console.log('No llegaron swaps en la ruta de Alcor para PUSSY.');
    return false;
  }
  ...
  for (let i = 0; i < swaps.length; i++) {
    const swap = swaps[i] || {};
    const inputRaw = String(swap.input || '').trim();
    const memo = String(swap.memo || '').trim();

    if (!inputRaw || !memo) {
      console.log(`Swap PUSSY inválido en tramo ${i + 1}: falta input o memo.`);
      continue;
    }

    let quantity = inputRaw;
    if (!quantity.includes(' ')) {
      const formatted = formatXmdQuantity(inputRaw);
      if (!formatted) {
        console.log(`Swap PUSSY inválido en tramo ${i + 1}: input no numérico (${inputRaw}).`);
        continue;
      }
      quantity = formatted;
    }

    console.log(`[PUSSY SWAP ${i + 1}/${swaps.length}] ${quantity} -> ${alcorSwapAccount}`);

    const result = await transactWithRetry([
      {
        account: xmdTokenContract,
        name: 'transfer',
        authorization,
        data: {
          from: username,
    ...
    boughtAny = true;
    console.log(`[PUSSY SWAP ${i + 1}/${swaps.length}] TX: ${result.transaction_id || 'N/A'}`);
  }

  if (boughtAny) {
    markBoughtMarketSymbol(pussyMarketSymbol);
  }
...
const getChangePercentage = (trade) => {
  ...
  return Number.isFinite(num) ? truncateToDecimals(num, 2) : null;
};
...
const executeBuyCycle = async (balanceAmount) => {
  ...
  if (allocationSymbols.includes(pussyMarketSymbol) || allocationSymbols.includes('PUSSY_XUSDC')) {
    try {
      const pussyInfo = await fetchPussyMarketInfo();
      if (pussyInfo.change24h !== null) {
        const pussyChange24h = truncateToDecimals(pussyInfo.change24h, 2);
        // Regla: la compra de PUSSY se ejecuta en mercado XMD, pero la referencia de precio
        // externa se toma del par PUSSY/XUSDC en Bloks.
        tradesBySymbol[pussyMarketSymbol] = {
          symbol: pussyMarketSymbol,
          change_percentage: pussyChange24h,
        };

        if (allocationSymbols.includes('PUSSY_XUSDC')) {
          tradesBySymbol.PUSSY_XUSDC = {
            symbol: 'PUSSY_XUSDC',
            change_percentage: pussyChange24h,
          };
        }

        if (pussyInfo.xusdcPrice !== null) {
          console.log(`PUSSY referencia XUSDC: ${pussyInfo.xusdcPrice} | change_24hr global (2 decimales): ${pussyChange24h}%`);
        } else {
          console.log(`PUSSY change_24hr global usado (2 decimales): ${pussyChange24h}%`);
        }
      } else {
        console.log('No se pudo leer change_24hr global de PUSSY; se omite para reglas de caída.');
      }
    } catch (error) {
      console.log('Error leyendo info externa de PUSSY:', error.message || error);
    }
  }

  const blockedByCalendar = allocationSymbols.filter(symbol => !isSymbolAllowedNow(symbol));
  const activeByCalendar = allocationSymbols.filter(symbol => isSymbolAllowedNow(symbol));

  if (!activeByCalendar.length) {
    console.log('Todos los mercados con asignación > 0 están bloqueados por calendario mensual. No se compra hoy.');
    return;
  }
  ...
  const blockedAllocationTotal = blockedByCalendar.reduce((sum, symbol) => {
    const allocation = Number(marketAllocations[symbol]);
    return sum + (Number.isFinite(allocation) ? allocation : 0);
  }, 0);

  const extraPerActiveSymbol = activeByCalendar.length > 0
    ? blockedAllocationTotal / activeByCalendar.length
    : 0;

  const effectiveAllocations = Object.fromEntries(
    allocationSymbols.map((symbol) => {
      const base = Number(marketAllocations[symbol]);
      if (!isSymbolAllowedNow(symbol)) {
        return [symbol, 0];
      }
      return [symbol, base + extraPerActiveSymbol];
    })
  );
  ...
  const symbolsAtMinusFifty = allocationSymbols.filter(symbol => {
    ...
    const change = getChangePercentage(trade);
    return change !== null
      && change <= umbralCaidaExtremaPct
      && isSymbolAllowedNow(symbol)
        && symbol !== pussyMarketSymbol
        && symbol !== 'PUSSY_XUSDC';
  });
  ...
  const divisorDias = Math.max(remainingDays, 1);
  const allActiveInNegative = activeByCalendar.length > 0 && negativeSymbols.length === activeByCalendar.length;
  const totalForAllocation = (remainingDays <= 0 && allActiveInNegative)
    ? Math.max(0, balanceAmount - 1)
    : balanceAmount;
  const dailyTotal = parseFloat((totalForAllocation / divisorDias).toFixed(2));

  if (remainingDays <= 0) {
    if (allActiveInNegative) {
      console.log(`Último día del mes y todos en negativo: se usa balance - 1 XMD para asignación (${totalForAllocation}).`);
    } else {
      console.log('Último día del mes: se usará el balance diario completo según porcentajes y condiciones.');
    }
  }
  ...
  for (const symbol of negativeSymbols) {
    const allocation = Number(effectiveAllocations[symbol]);
    ...
    if (symbol === pussyMarketSymbol) {
      promises.push(
        executePussyBuyViaSwap(symbolQty).catch((error) => {
          console.log(`Error en compra PUSSY por swap: ${error.message || error}`);
          return false;
        })
      );
      continue;
